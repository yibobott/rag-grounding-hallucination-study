"""
E2: Dense Retrieval with Contriever
=====================================
For each HotpotQA question, Contriever ranks the 10 distractor-setting context
documents and selects the top-5. These are fed to GPT-4o-mini for generation.

Usage:
    python -m experiments.e2_dense [--sample_size 500] [--model gpt-4o-mini] [--dry_run]
...
def _make_prepare_dense(top_k: int):
    """Return a prepare_fn that uses dense retrieval with the given top_k."""

    def _prepare(example: dict) -> dict:
        all_docs = get_all_context_docs(example)
        gold_titles = get_gold_titles(example)
        retrieved_docs = dense_retrieve(
...
def run_e2_dense(
    ...
    """Run the E2 dense retrieval experiment with Contriever."""
    if output_dir is None:
        model_dir = model_key.replace("/", "_")
        output_dir = config.OUTPUT_DIR / "e2_dense" / model_dir
    ...
    run_cfg = {
        ...
        "description": "Dense retrieval baseline — Contriever re-rank 10 distractor docs, top-5",
    }

    return run_experiment(
        experiment_name="E2-Dense",
        prepare_fn=_make_prepare_dense(top_k),
        run_cfg=run_cfg,
        sample_size=sample_size,
        model_key=model_key,
...
def main():
    parser = argparse.ArgumentParser(description="E2: Dense retrieval with Contriever")
